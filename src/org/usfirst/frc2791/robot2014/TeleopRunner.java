...
public class TeleopRunner {
  ...
  private void driveTeleop() {
    ...
    //    Robot2014.driveTrain.setLeftRightSpeed(scaleJoysticks(Robot2014.joystickLeft.getY()),
    //      scaleJoysticks(joystickRight.getY()));
    //
    // tank drive
  ...
  private void armTeleop() {
    ...
    if (armManualMovement > OPEREATOR_STICK_DEAD_ZONE || armManualMovement < -OPEREATOR_STICK_DEAD_ZONE) {
  ...
  private void shooterClawTeleop() {
    ...
    if (autoCatch) { //auto catch
      ...
      if (intakeOut) //out
      {
        ...
        Robot2014.intakeClaw.setIntakeRollerOutput(
          baseSpeed + Robot2014.driveTrain.getSpeed() / 160.0 * (1.0 - baseSpeed));
      } else {
